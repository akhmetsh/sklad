...
Two versions are kept in [`docs/`](docs/):

### Simplified — recommended for documentation

![ERD simplified](./docs/ERD-simplified.svg)

11 main entities with key fields and the most important relationships. Item tables and the `AuditLog` are intentionally omitted (the schema noise distracts from the overall design). Hand-curated, sourced from [`docs/ERD-simplified.mmd`](docs/ERD-simplified.mmd).

```bash
npm run db:erd:simple    # regenerate after editing .mmd
```

### Full — auto-generated, useful for code review

[![Full ERD](./docs/ERD.svg)](./docs/ERD.svg)

Every model, every column, every foreign key — generated by [`prisma-erd-generator`](https://github.com/keonik/prisma-erd-generator) directly from `prisma/schema.prisma`.

```bash
npm run db:erd           # regenerate after schema change
```
