...
interface ItemRow { productId: string; storageLocationId: string; quantity: string; }

export default function DemoIssueNew() {
  ...
  const [items, setItems] = useState<ItemRow[]>([{ productId: "", storageLocationId: "", quantity: "" }]);
  ...
  function getAvailable(productId: string, locationId: string) {
    return stock.find((b) => b.productId === productId && b.warehouseId === warehouseId && b.storageLocationId === locationId)?.quantity ?? 0;
  }
  ...
  function addItem() { setItems((r) => [...r, { productId: "", storageLocationId: "", quantity: "" }]); }
  function removeItem(i: number) { setItems((r) => r.filter((_, idx) => idx !== i)); }

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!warehouseId || !recipientName.trim()) { setError("Заполните склад и получателя"); return; }
    if (items.some((r) => !r.productId || !r.storageLocationId || !r.quantity)) { setError("Заполните все строки товаров"); return; }

    const nextNum = `ISS-${1000 + state.issues.length + 1}`;
    const docItems: DocItem[] = items.map((r) => ({
      id: genId(), productId: r.productId, storageLocationId: r.storageLocationId, quantity: parseFloat(r.quantity),
    }));

    dispatch({
      type: "CREATE_ISSUE",
      doc: { id: genId(), documentNumber: nextNum, warehouseId, recipientName: recipientName.trim(), date, status: "DRAFT", comment: comment.trim() || undefined, createdById: "u-store", createdAt: now(), items: docItems },
    });
    dispatch({ type: "NOTIFY", message: `Документ ${nextNum} создан`, kind: "success" });
    router.push("/demo/documents/issues");
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <h1 className="text-xl font-semibold">Новая выдача</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <F label="Склад *">
              <select value={warehouseId} onChange={(e) => { setWarehouseId(e.target.value); setItems([{ productId: "", storageLocationId: "", quantity: "" }]); }} className={inp} required>
                <option value="">— выберите —</option>
                {state.warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
              </select>
            </F>
            <F label="Получатель *"><input value={recipientName} onChange={(e) => setRecipientName(e.target.value)} className={inp} required placeholder="Отдел, сотрудник или организация" /></F>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-3">
          ...
          <div className="space-y-2">
            {items.map((row, i) => {
              const available = row.productId && row.storageLocationId ? getAvailable(row.productId, row.storageLocationId) : null;
              ...
              return (
                <div key={i} className="space-y-1">
                  ...
                  {available !== null && (
                    <p className={`text-xs ml-0 ${insufficient ? "text-red-600" : "text-gray-500"}`}>
                      {insufficient ? `⚠ Недостаточно: доступно ${available}` : `Доступно: ${available}`}
                    </p>
                  )}
                </div>
              );
