...
export default function DemoIssueDetail({ params }: { params: { id: string } }) {
  ...
  function getAvailable(productId: string, locationId: string) {
    return stock.find((b) => b.productId === productId && b.warehouseId === doc!.warehouseId && b.storageLocationId === locationId)?.quantity ?? 0;
  }

  const canConfirm = doc.status === "DRAFT" && doc.items.every((item) => getAvailable(item.productId, item.storageLocationId) >= item.quantity);

  function confirm() {
    dispatch({ type: "CONFIRM_ISSUE", id: doc!.id });
  }
        ...
        <table className="min-w-full divide-y divide-gray-100 text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {doc.items.map((item) => {
              ...
              const available = getAvailable(item.productId, item.storageLocationId);
