...
export default async function ReceiptsPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Поступления" description="Документы поступления товара">
        <Link href="/documents/receipts/new" className="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700">
          + Создать
        </Link>
      </PageHeader>
        ...
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-medium text-gray-500">Создал</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {docs.map((d) => (
              <tr key={d.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-gray-600">{d.createdBy.name}</td>
                <td className="px-4 py-3 text-right">
                  <Link href={`/documents/receipts/${d.id}`} className="text-blue-600 hover:underline text-xs">
                    Открыть
                  </Link>
                </td>
              </tr>
