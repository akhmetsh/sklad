...
export default async function ReceiptDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <PageHeader title={`Поступление ${doc.documentNumber}`} backHref="/documents/receipts">
        {doc.status === "DRAFT" && (
          <ConfirmDocumentButton
            docId={doc.id}
            type="receipt"
            label="Подтвердить поступление"
          />
        )}
      </PageHeader>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 grid grid-cols-2 gap-4 text-sm">
        ...
        <Field label="Поставщик">{doc.supplier.name}</Field>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-200 font-medium">Позиции</div>
