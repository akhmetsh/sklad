...
export default async function TransfersPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Перемещения" description="Внутренние перемещения товара">
        <Link href="/documents/transfers/new" className="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700">
          + Создать
        </Link>
      </PageHeader>
          ...
          <tbody className="divide-y divide-gray-100">
            {docs.map((d) => (
              <tr key={d.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">{d.fromWarehouse.name}</td>
                <td className="px-4 py-3">{d.toWarehouse.name}</td>
                ...
                <td className="px-4 py-3 text-right">
                  <Link href={`/documents/transfers/${d.id}`} className="text-blue-600 hover:underline text-xs">Открыть</Link>
                </td>
              </tr>
            ))}
          </tbody>
