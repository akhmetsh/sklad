...
interface Transfer {
  ...
  fromWarehouseName: string; toWarehouseName: string;
  itemsCount: number; status: string; createdByName: string;
}

export function TransfersClient({ initial }: { initial: Transfer[] }) {
  ...
  function handleExport() {
    const headers = [
      ...
      t.documents.transfers.fields.fromWarehouse, t.documents.transfers.fields.toWarehouse,
      t.documents.common.itemsCount, t.common.status, t.audit.columns.user,
    ];
    const rows = filtered.map((r) => [
      r.documentNumber, formatDate(r.date), r.fromWarehouseName, r.toWarehouseName,
      r.itemsCount, r.status === "DRAFT" ? t.status.draft : r.status === "CONFIRMED" ? t.status.confirmed : t.status.cancelled,
      r.createdByName,
    ]);
    downloadCSV(`transfers-${new Date().toISOString().split("T")[0]}.csv`, toCSV(headers, rows));
  }
  ...
  const columns: Column<Transfer>[] = [
    {
      ...
      cell: (r) => (
        <Link href={`/documents/transfers/${r.id}`} className="font-mono text-sm font-medium text-brand-600 hover:underline">
          {r.documentNumber}
        </Link>
      ),
    },
  ...
  return (
    <div className="space-y-4">
      <PageHeader title={t.documents.transfers.title} description={t.documents.transfers.subtitle}>
        <Button variant="secondary" onClick={handleExport} disabled={filtered.length === 0}>
          <Download className="w-4 h-4" /> CSV
        </Button>
        <Link href="/documents/transfers/new">
          <Button><Plus className="w-4 h-4" /> {t.documents.transfers.create}</Button>
        </Link>
      </PageHeader>
      ...
      <DataTable
        ...
        data={filtered}
        rowKey={(r) => r.id}
        onRowClick={(r) => router.push(`/documents/transfers/${r.id}`)}
        empty={<EmptyState
          title={t.common.noData}
          action={<Link href="/documents/transfers/new"><Button><Plus className="w-4 h-4" /> {t.documents.transfers.create}</Button></Link>}
        />}
      />
    </div>
  );
