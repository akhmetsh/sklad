...
export function TransferDocumentForm({ warehouses, products, locations }: Props) {
  const router = useRouter();
  const { register, handleSubmit, watch, control, formState: { errors, isSubmitting } } = useForm<TransferDocumentInput>({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-6 grid grid-cols-2 gap-4">
        <Field label="Склад отправления *" error={(errors as { fromWarehouseId?: { message?: string } }).fromWarehouseId?.message}>
          <select {...register("fromWarehouseId")} className={inp}>
            <option value="">— выберите —</option>
            {warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
          </select>
        </Field>
        <Field label="Склад назначения *" error={(errors as { toWarehouseId?: { message?: string } }).toWarehouseId?.message}>
          <select {...register("toWarehouseId")} className={inp}>
            <option value="">— выберите —</option>
            {warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
          </select>
        </Field>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
          ...
          <button type="button" onClick={() => append({ productId: "", fromStorageLocationId: "", toStorageLocationId: "", quantity: 1 })} className="text-blue-600 text-sm hover:underline">
            + Добавить строку
          </button>
        </div>
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-3 py-2 text-left font-medium text-gray-500">Откуда (место)</th>
              <th className="px-3 py-2 text-left font-medium text-gray-500">Куда (место)</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {fields.map((field, i) => (
              <tr key={field.id}>
                ...
                <td className="px-3 py-2">
                  <select {...register(`items.${i}.fromStorageLocationId`)} className={inp} disabled={!fromWarehouseId}>
                    <option value="">— место —</option>
                    {fromLocations.map((l) => <option key={l.id} value={l.id}>{l.code} — {l.name}</option>)}
                  </select>
                </td>
                <td className="px-3 py-2">
                  <select {...register(`items.${i}.toStorageLocationId`)} className={inp} disabled={!toWarehouseId}>
                    <option value="">— место —</option>
