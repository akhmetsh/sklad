...
export function LocationCreateForm({ warehouses }: Props) {
    ...
    <div>
      ...
      {open && (
        <form onSubmit={handleSubmit} className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3 max-w-lg">
          <p className="text-sm font-medium text-blue-900">Новое место хранения</p>

          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Склад *</label>
            <select name="warehouseId" required className={inp}>
              <option value="">— выберите склад —</option>
              {warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Код (напр. A-01) *</label>
            <input name="code" required className={inp} placeholder="A-01" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Название *</label>
            <input name="name" required className={inp} placeholder="Стеллаж A, полка 1" />
          </div>
          <div>
